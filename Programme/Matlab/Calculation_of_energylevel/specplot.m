function specplot(Ej,Ec,alpha,beta,kappa,sigma,nk,nl,nm,nlevels)
%     examclcple:specplot(50,1,0.43,0,0,0,5,10,2,6)
...
%     hold on;
   ...
   parfor i=1:41
       EL = TriJFlxQbtEL(Ej,Ec,alpha,beta,kappa,sigma,0.48+(i-1)*0.001,nk,nl,nm,nlevels);
...
% Based on the paper: Robertson et al., Phys. Rev. Letts. B 73, 174526 (2006). 
% 
% Example:
				 ...
				 for k2=-nk:nk
					 ...
					 for l2=-nl:nl
						 ...
						 for m2=0:nm
							 ...
							 H2=-1i*sqrt((m_t*omega_t*hbar^3)/2)*(M(1,3)*k1*Kdelta(k1,k2)+M(2,3)*l1*Kdelta(l1,l2))*(sqrt(m2+1)*Kdelta(m2+1,m1)-sqrt(m2)*Kdelta(m2-1,m1));
							 tmp1=0;
							 for p=-1:1
							         row=p+2;
							 		for q=-2:2
