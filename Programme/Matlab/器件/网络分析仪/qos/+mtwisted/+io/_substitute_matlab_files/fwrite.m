function fwrite(obj, bytes, mode)
    % write bytes
    % mode 0, syncronized
    % mode 1, asyncronized

    % a simplified version of fwrite for speed
    ...
    try
       fwrite(igetfield(obj, 'jobject'), bytes, length(bytes), 5, mode, 0);
    catch aException
        throw(MException('instrument:fwrite:opfailed', aException.message));
    end
end
...
% %   $Revision: 1.1.6.3 $  $Date: 2013/11/07 20:40:29 $
% 
% % Error checking.
...
%     % g1003025 - Preallocating the array for better performance. 
%     tmp = javaArray('java.lang.Long', length(cmd));    
%     for iLoop = 1:length(cmd)
%         tmp(iLoop) = Long(cmd(iLoop)); 
%     end
%     cmd=tmp;
