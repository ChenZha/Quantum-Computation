...
%                'I','I','I','I','Y2p'};
   ...
   phase989 = qes.expParam(aczSettings1,'dynamicPhases(1)');
   phase988 = qes.expParam(aczSettings1,'dynamicPhases(2)');
   
   phase787 = qes.expParam(aczSettings2,'dynamicPhases(1)');
   phase788 = qes.expParam(aczSettings2,'dynamicPhases(2)');
   phase786 = qes.expParam(aczSettings2,'dynamicPhases(3)');
   
   phase767 = qes.expParam(aczSettings3,'dynamicPhases(1)');
   phase766 = qes.expParam(aczSettings3,'dynamicPhases(2)');
   phase768 = qes.expParam(aczSettings3,'dynamicPhases(3)');
   
   phase565 = qes.expParam(aczSettings4,'dynamicPhases(1)');
   phase566 = qes.expParam(aczSettings4,'dynamicPhases(2)');
...
%    R.datafcn = @(x) sum(x(2:end-1))-x(1)-x(end);
   ...
   tolY = [5e-3];
    ...
    datafile = fullfile(QS.loadSSettings('data_path'),...
            ['5QGHZ_beforeOpt',timeStamp,rndNum,'_.mat']);
    figfile = fullfile(QS.loadSSettings('data_path'),...
            ['5QGHZ_beforeOpt',timeStamp,rndNum,'_.fig']);
  

%    %%%%%%%%%%%%%%%%%%%
%    procFactory();
%    probDataBeforeOpt = R();
%    h1 = qes.ui.qosFigure(sprintf('GHZ | '),false);
%    ax1 = axes(h1);
%    bar(ax1,probDataBeforeOpt);
%    xlabel(ax1,'|0...0>,|0...1>,...,|1...1>');
%    ylabel(ax1,'P');
%    title(ax1,'before optimization');
%    grid(ax1,'on');
%    drawnow;
%    Rtomo = stateTomography(qubits);
%    Rtomo.setProcess(sqc.op.physical.gateParser.parse(qubits,gateMat));
%    tomoDataBeforeOpt = Rtomo();
%    

%     save(datafile,'tomoDataBeforeOpt','probDataBeforeOpt')
%     if ishghandle(h1)
%         saveas(h1,figfile);
%     end
%     
...
%    phase566.val = 1.8239;
   ...
   ax1 = axes(h1);
   bar(ax1,probDataAfterOpt);
   xlabel(ax1,'|0...0>,|0...1>,...,|1...1>');
   ylabel(ax1,'P');
   title(ax1,'after optimization');
   grid(ax1,'on');
   drawnow;
   Rtomo = stateTomography(qubits);
   Rtomo.setProcess(sqc.op.physical.gateParser.parse(qubits,gateMat));
   tomoDataAfterOpt = Rtomo();
   
    datafile = fullfile(QS.loadSSettings('data_path'),...
            ['5QGHZ_afterOpt',timeStamp,rndNum,'_.mat']);
    figfile = fullfile(QS.loadSSettings('data_path'),...
            ['5QGHZ_afterOpt',timeStamp,rndNum,'_.fig']);
    save(datafile,'tomoDataAfterOpt','probDataAfterOpt')
    if ishghandle(h1)
        saveas(h1,figfile);
    end
