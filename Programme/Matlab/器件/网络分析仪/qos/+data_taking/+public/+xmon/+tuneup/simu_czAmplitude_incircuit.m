function varargout = simu_czAmplitude_incircuit(varargin)
...
% Yulin Wu, 2017/10/14
    ...
    args = util.processArgs(varargin,{'gui',false,'notes','','save',true,...
        'largeRange',false,'repeatIfOutOfBoundButClose',false,'logger',[],'fastMeasure',false,'czln',[]});
    ...
    if ~isempty(args.czln)
        scz.aczln=args.czln;
    end
    
	czAmp0 = scz.amp;
    if args.largeRange
        czAmp= round(scz.amp*linspace(0.7,1.3,50));
    elseif args.fastMeasure
        czAmp= round(scz.amp*linspace(0.95,1.05,30));
    else
        czAmp= round(scz.amp*linspace(0.98,1.02,30));
    end
%     try
        acz1= data_taking.public.xmon.simu_acz_ampLength_incircuit('controlQ',qc,'targetQ',qt,...
           'dataTyp','Phase',...
           'czLength',scz.aczLn,'czAmp',czAmp,'cState','1',...
           'notes','','gui',false,'save',false);
        acz0= data_taking.public.xmon.simu_acz_ampLength_incircuit('controlQ',qc,'targetQ',qt,...
           'dataTyp','Phase',...
...
%                 'Yes','No','No');
    ...
    if args.gui && ~repeat
        ...
        if args.save && ~isempty(hf) && isvalid(hf)
            ...
            try
                save(replace(dataSvName,'.fig','.mat'),'qc','qt','czamp','czAmp0','cz0data','cz1data')
    ...
    if args.repeatIfOutOfBoundButClose && repeat
        czamp=data_taking.public.xmon.tuneup.simu_czAmplitude_incircuit(...
