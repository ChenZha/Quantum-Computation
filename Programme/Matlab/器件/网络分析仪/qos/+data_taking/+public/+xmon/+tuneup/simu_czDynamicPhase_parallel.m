function varargout = simu_czDynamicPhase_parallel(varargin)
% <_o_> = simu_czDynamicPhase_parallel('controlQ',_c&o_,'targetQ',_c&o_,'dynamicPhaseQ',[_c&o_],...
...
% GM, 20180423
    ...
    fcn_name = 'data_taking.public.xmon.tuneup.simu_czDynamicPhase_parallel'; % this and args will be saved with data
    import qes.*
    import sqc.*
    ...
    args = util.processArgs(varargin,{'numCZs',10,'PhaseTolerance',0.02,'numIter',3,'gui',false,'notes','','save',true});
    ...
    dynamicPhaseQs = setdiff(czs.qubits,[simutq,simucq,qc.name,qt.name]);
%     dynamicPhaseQs = args.dynamicPhaseQs;
    ...
    count = 0;
    repeat=true;
    while repeat && count < args.numIter
        CZ = gate.CZ(qc,qt);
...
%         data = toolbox.data_tool.unwrap_plus(data,[],1);
        data = unwrap(data,[],1);
        x = x(:);
        repeat=false;
        for jj = 1:numQs
            if args.numCZs>1
                ...
                if args.gui
                    hf = qes.ui.qosFigure(sprintf('ACZ dynamic phase | %s,%s,%s',...
                        qc.name, qt.name, dynamicPhaseQs{jj}.name),true);
                    ax = axes('parent',hf);
            ...
            if abs(p(1)) > args.PhaseTolerance
                repeat=true;
            end
        end
    end
