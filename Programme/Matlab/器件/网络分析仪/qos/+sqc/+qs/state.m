classdef state
    ...
    % ==================================================================
	% usage:
	% qsobj = state('0.55|1000>+0.86603|0101>');
    % qsobj = state([0,1,i,0]);
	% outstate = U*instate; here U is sqc.op.logical.operator class object(a quantum gate),
    % instate, outstate are state class objects(quantum states)
    % normalization is not neccessary, state will do it
...
% mail4ywu@gmail.com/mail4ywu@icloud.com
    ...
    methods
        ...
        function W = DMatrix(obj)
            % convert quantum state(state) to density matrix
            % output is sqc.op.logical.operator class object
            W = sqc.op.logical.operator(kron(obj.v',obj.v));
        end
    end
    methods (Hidden = true)
        function W = mtimes(U,V) % implement * as Kronecker tensor product
            if isa(V,'sqc.qs.state')
                W = sqc.qs.state(kron(U.v,V.v));
            else
                error('V not a sqc.qs.state class object');
            end
        end
    end
    methods (Static = true)
