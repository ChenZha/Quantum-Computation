...
if ~libisloaded('ATSApi')    
    if strcmpi(computer('arch'), 'win64') 
        ...
        loadlibrary('ATSApi.dll',@qes.hwdriver.sync.alazarATS.AlazarInclude_pcwin64)
    elseif sscanf(version('-release'), '%d') >= 2009
        % Use protofile for 32-bit MATLAB 2009 and later
        loadlibrary('ATSApi.dll',@qes.hwdriver.sync.alazarATS.AlazarInclude_pcwin32)
    else
        % Use protofile for 32-bit versions of MATLAB ealier than 2009
        loadlibrary('ATSApi.dll',@qes.hwdriver.sync.alazarATS.AlazarInclude)                
    end
    if libisloaded('ATSApi')
        result = true;
    end
