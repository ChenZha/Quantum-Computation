...
if nargin >= 2
    ...
    elseif isa(whichbar, 'matlab.ui.container.Panel')
...
switch type
    case 1
		...
		if x < oldxpatch(2)
			% Warning: The EraseMode property is no longer supported and will error in a future release. Use the ANIMATEDLINE function for animating lines and points instead of EraseMode 'none'. Removing
            % instances of EraseMode set to 'normal', 'xor', and 'background' has minimal impact. 
%             warning('off');
...
% 		set(p,'EraseMode','none')
	...
	case 3
        ...
        % Display text on top of axes
          tHandle       = title(ha,waitbartext,'FontUnits','normalized','FontSize',text_to_bar_height);
...
function draw_progress_bar(f,ha,x)
	...
	ybackground   = [0 0 1 1];
	p_color = b_map(floor(x / 100 * (size(b_map,1)-1)) + 1, :);
	l_color = get(ha, 'XColor');
% 	p       = patch(xprogress, yprogress, p_color, 'Tag', 'progress', 'EdgeColor', 'none', 'EraseMode', 'none','Parent',ha);
    p       = patch(xprogress, yprogress, p_color, 'Tag', 'progress', 'EdgeColor', 'none','Parent',ha);
    l       = patch(xbackground, ybackground, 'w', 'Tag', 'background', 'FaceColor', 'none', 'EdgeColor', l_color,'Parent',ha);
% 	l       = patch(xbackground, ybackground, 'w', 'Tag', 'background', 'FaceColor', 'none', 'EdgeColor', l_color, 'EraseMode', 'none','Parent',ha);
