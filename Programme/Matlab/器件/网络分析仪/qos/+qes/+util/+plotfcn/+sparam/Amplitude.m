            ...
            elseif any(sz==2) % 2 by N or N by 2 data, sparamter and frequency
                if sz(1) == sz(2) 
                    ...
                    elseif all(isreal(Data(1,:))) && all(isreal(Data(2,:)))
                        % confusing case, frequencies are normally much greater than s parameters, take a resonable guess
                    ...
                    elseif all(~isreal(Data(1,:))) && any(isreal(Data(2,:)))
                        freq = Data(2,:);
                        sparam = Data(1,:);
                    else % confusing case, frequencies are normally much greater than s parameters, take a resonable guess
    ...
    elseif length(SweepVals{1}{MainParam(1)}) > 1 % single sweep, multipoint sweep point measurement
        ...
        if all(sz > 2) || length(sz) > 2 
            ...
            if all(sz>=2) % data can only be cell
               ...
               % frequencies are normally much greater than s parameters, take a resonable guess
