classdef SonnetCircuitPhysicalTransmissionLine < handle
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % This class defines a physical transmission line element in
    % the circuit (CKT) block of a Sonnet Netlist project.
    %
    ...
    properties
        ...
        EeffValue           % The eeff value of the transmission line
        AttenuationValue   % The attentuation of the transmission line
    ...
    methods
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function obj = SonnetCircuitPhysicalTransmissionLine(theFid,theNetworkNumber)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if nargin == 2
                ...
                % Read in the length
                fscanf(theFid,' %c',1);                                 % Get the string 'L', we dont need to store it anywhere
                fscanf(theFid,' %c',1);                                 % Get the string '=', we dont need to store it anywhere
                ...
                % Read in the eeff
                fscanf(theFid,' %c',1);                                 % Get the string 'K', we dont need to store it anywhere
                fscanf(theFid,' %c',1);                                 % Get the string '=', we dont need to store it anywhere
                
                aBackupOfTheFid=ftell(theFid);                          % Store a backup of the file ID so that we can restore it afer we read the line
                aTempCharacter=fscanf(theFid,' %s',1);                  % Read in a temporary character, if it is a number then the eeff was represented as an FP number, otherwise it is a variable
                fseek(theFid,aBackupOfTheFid,'bof');                    % Restore the backup of the fid
                
                if isnan(str2double(aTempCharacter))
                    obj.EeffValue=fscanf(theFid,' %s',1);
                    obj.EeffValue=strrep(obj.EeffValue,'=','');
                    obj.EeffValue=strtrim(obj.EeffValue);
                    if ~isnan(str2double(obj.EeffValue))
                        obj.EeffValue=str2double(obj.EeffValue);
                    end
                else
                    obj.EeffValue=fscanf(theFid,' %f',1);
                end
                
                % Read in the frequency
                aBackupOfTheFid=ftell(theFid);                          % Store a backup of the file ID so that we can restore it afer we read the line
                aTempCharacter=fscanf(theFid,' %c',1);                  % Read in a temporary character, if it is a number then the impedance was represented as an FP number, otherwise it is a variable
                fseek(theFid,aBackupOfTheFid,'bof');                    % Restore the backup of the fid
                
                if strcmp(aTempCharacter,'F')==1
                    ...
                    % Read in the attentuation
                    fscanf(theFid,' %c',1);                             % Get the string 'A', we dont need to store it anywhere
                    fscanf(theFid,' %c',1);                             % Get the string '=', we dont need to store it anywhere
                    
                    aBackupOfTheFid=ftell(theFid);                      % Store a backup of the file ID so that we can restore it afer we read the line
                    aTempCharacter=fscanf(theFid,' %s',1);              % Read in a temporary character, if it is a number then the attentuation was represented as an FP number, otherwise it is a variable
                    fseek(theFid,aBackupOfTheFid,'bof');                % Restore the backup of the fid
                    
                    if isnan(str2double(aTempCharacter))
                        obj.AttenuationValue=fscanf(theFid,' %s',1);
                        obj.AttenuationValue=strrep(obj.AttenuationValue,'=','');
                        obj.AttenuationValue=strtrim(obj.AttenuationValue);
                        if ~isnan(str2double(obj.AttenuationValue))
                            obj.AttenuationValue=str2double(obj.AttenuationValue);
                        end
                    else
                        obj.AttenuationValue=fscanf(theFid,' %f',1);
                    end
        ...
        function aNewObject=clone(obj)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            aNewObject=SonnetCircuitPhysicalTransmissionLine();
        ...
        function aSignature=stringSignature(obj,theVersion) %#ok<INUSD>
            ...
            if ~isempty(obj.LengthValue)
                if isa(obj.LengthValue,'char')
                    aSignature = [aSignature sprintf(' L=%s',obj.LengthValue)];
                else
                    aSignature = [aSignature sprintf(' L=%.15g',obj.LengthValue)];
                end
            end
            
            if ~isempty(obj.EeffValue)
                if isa(obj.EeffValue,'char')
                    aSignature = [aSignature sprintf(' K=%s',obj.EeffValue)];
                else
                    aSignature = [aSignature sprintf(' K=%.15g',obj.EeffValue)];
                end
            end
            ...
            if ~isempty(obj.AttenuationValue)
                if isa(obj.AttenuationValue,'char')
                    aSignature = [aSignature sprintf(' A=%s',obj.AttenuationValue)];
                else
                    aSignature = [aSignature sprintf(' A=%.15g',obj.AttenuationValue)];
                end
            end
