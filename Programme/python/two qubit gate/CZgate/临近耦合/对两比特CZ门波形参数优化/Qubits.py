...
class Qubits():
    ...
    def process(self , drive = None , process_plot  = False , RWF = 'CpRWF' , parallel = False , argument = {'T_p':100,'T_copies':201} , options = default_options):
        ...
        basic , loc = self._basic_generation_3()
        ...
        process = np.column_stack([final_state[i].data.toarray() for i in range(len(final_state))]) #只取演化矩阵中二能级部分
    ...
    def _basic_generation(self):
        ...
        return(basic,loc)
    def _basic_generation_3(self):
        '''生成3^n个基矢,以及各个基矢在3能级系统中的位置'''
        basic = []
        loc = []
        for index in range(3**self.num_qubits):
            II = index
            code = '' #转化成二进制的字符串形式
            state = [] #生成一个基矢
            l = 0 #在3能级中的位置
            for JJ in range(self.num_qubits):
                number = np.int(np.mod(II,3))
                code += str(number)
                state.insert(0,basis(self.N_level , number))
                l += number*self.N_level**JJ
                II = np.int(np.floor(II/3))
            assert len(code) == len(state) == self.num_qubits    
