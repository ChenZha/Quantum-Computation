...
#    psi_target=(tensor(basis(3,0),basis(3,0))+tensor(basis(3,0),basis(3,1))+tensor(basis(3,1),basis(3,0))-tensor(basis(3,1),basis(3,1))).unit()
    ...
    fid=fminbound(getfid,700,850, xtol=1e-07,disp=3)
...
if __name__=='__main__':#fminbound
    ...
    ## Qubits frequency
    w_q = np.array([ 5.0 , 6.2 , 5.4 , 5.6 , 5.8 , 4.0 , 4.2 , 4.4 , 4.6 , 4.8 ]) * 2 * np.pi
...
#        psi0=tensor(basis(N,0), (basis(3,1)+basis(3,0)).unit(), (basis(3,1)).unit(), (basis(3,0)+basis(3,1)).unit(), basis(3,0) ,basis(3,1))
        psi0=tensor(basis(N,0), (basis(3,1)-basis(3,0)).unit(), (basis(3,1)+1j*basis(3,0)).unit(), (basis(3,0)+basis(3,1)).unit(), basis(3,0) ,basis(3,1))
    elif APRType==1:
...
#    options.rtol=1e-9
    ...
    measurement0 = Measure(psi_total[-1],0)
