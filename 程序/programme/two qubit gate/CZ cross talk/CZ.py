...
def evolution(H,psi,tlist,args,options):
    psi0 = psi[0]
    target = psi[1]
...
#    psi0 = tensor( (basis(3,0)).unit() , (basis(3,1)).unit())
    ...
    psi = []
    t = []
    t.append((bastate[0]).unit());t.append((bastate[1]).unit());t.append((bastate[0]-bastate[1]).unit());t.append((bastate[0]-1j*bastate[1]).unit())
    t.append((bastate[2]).unit());t.append((bastate[4]).unit());t.append((bastate[2]-bastate[4]).unit());t.append((bastate[2]-1j*bastate[4]).unit())
    psi.append(t)
    t = []
    t.append((bastate[0]).unit());t.append((bastate[1]).unit());t.append((bastate[0]-bastate[1]).unit());t.append((bastate[0]-1j*bastate[1]).unit())
    t.append((bastate[2]).unit());t.append(-(bastate[4]).unit());t.append((bastate[2]+bastate[4]).unit());t.append((bastate[2]+1j*bastate[4]).unit())
...
#    result = mesolve(H,psi0,tlist,[],[],args = args,options = options)
    evo = lambda psi:evolution(H,psi,tlist,args,options)
...
#    E = CZgate([38.68574872,-4.74773091])
    ...
    x0 = [np.pi/np.sqrt(2)/g+10,(w_q[1]-w_q[0]+eta_q[1])]
