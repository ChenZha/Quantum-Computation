...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 25 20:08:54 2017
...
def getfid(T):
    ...
    U1 = basis(N,0)*basis(N,0).dag()+np.exp(1j*(E[1]-E[0]+E[5]-E[2])/2*tlist[-1])*basis(N,1)*basis(N,1).dag()
    U2 = basis(N,0)*basis(N,0).dag()+np.exp(1j*(E[2]-E[0]+E[5]-E[1])/2*tlist[-1])*basis(N,1)*basis(N,1).dag()
    UT = tensor(U1,U2)
...
#==============================================================================
    ...
    for t in range(0,len(tlist)):
        U0 = basis(3,0)*basis(3,0).dag()+np.exp(1j*(E[1]-E[0]+E[5]-E[2])/2*tlist[t])*basis(3,1)*basis(3,1).dag()
        U1 = basis(3,0)*basis(3,0).dag()+np.exp(1j*(E[2]-E[0]+E[5]-E[1])/2*tlist[t])*basis(3,1)*basis(3,1).dag()
        U = tensor(U0,U1)
...
#    omega = 0.060*2*np.pi
    ...
    f1 = (E[2]-E[0]+E[5]-E[1])/2
    f2 = (E[1]-E[0]+E[5]-E[2])/2
#    f1 = E[2]-E[0]#CR驱动频率
#    f2 = E[1]-E[0]#X波频率
    alpha = 0.05
...
if __name__=='__main__':
    ...
    g = 0.0038 * 2 * np.pi
    wq= np.array([4.914 , 5.1914 ]) * 2 * np.pi
...
#    fid = CNOT([70.1805137306,1.10522763])
    fid = CNOT([104.7520927  ,   0.63144823])
#    fid = CNOT([67.5,0.060*2*np.pi])
#    print(fid)
