...
#==============================================================================
    ...
    w = interpolate.interp1d(tt,delta*th,'slinear')
    for i,t in enumerate(tlist):
        H1 = H0+sn[0]*(w(t).tolist())
        S = H1.eigenstates()[1]
        s11 = S[findstate(S,'11')]
        s02 = S[findstate(S,'02')]
        l11.append( fidelity(s11,result.states[i]) )
        l02.append( fidelity(s02,result.states[i]) )
    
    figure();plot(tlist,l11);xlabel('t');ylabel('P11')
    figure();plot(tlist,l02);xlabel('t');ylabel('P02')
#==============================================================================
#==============================================================================
...
##        U0 = basis(3,0)*basis(3,0).dag()+np.exp(1j*(Ee[2]-Ee[0])*tlist[t])*basis(3,1)*basis(3,1).dag()
##        U1 = basis(3,0)*basis(3,0).dag()+np.exp(1j*(Ee[1]-Ee[0])*tlist[t])*basis(3,1)*basis(3,1).dag()
##        U = tensor(U0,U1)
...
#    target = (bastate[4]).unit()
    ...
    l20 = expect(E_uc[0],result.states)
    l21 = expect(E_uc[1],result.states)
#    figure();plot(tlist,l20);xlabel('t');ylabel('P2');title('Q0')
#    figure();plot(tlist,l21);xlabel('t');ylabel('P2');title('Q1')
    print('fidelity = ',fid,P,l20[-1],l21[-1])
    #==============================================================================
    return(fid)
...
#    x0 = [1.5*np.pi/np.sqrt(2)/g,(w_q[1]-w_q[0]+eta_q[1])]
#    result = minimize(CZgate, x0, method="Nelder-Mead",options={'disp': True})
#    print(result.x[0],result.x[1])

#==============================================================================
#    t = np.arange(20,35,1)
#    Am = np.arange(1.514,1.554,0.001)
...
#    np.save('No_CrossTalk_1_0+1_20-35',z)    
##==============================================================================
#    t = 98
#    Am = np.linspace(0.9*1.272,1.1*1.272,201)
