...
def opt(tar):
    ...
    #    print(ome,width)
        ...
        eta_q = np.array([-0.345 , -0.244]) * 2 * np.pi
        #    w_q12 = deltaw+w_c
    ...
    result=fminbound(Two_GeoCZ,7.0,14.0, xtol=1e-07,full_output = True)
    print(tar,result[0],1-result[1])
...
if __name__ == '__main__':
    ...
    starttime = clock()
#     fid = Two_GeoCZ([10.0938022135；29.0594713763]
...
#    result = minimize(Two_GeoCZ, x0, method="powell",options={'disp': True})
    ...
    g = np.linspace(0.0,0.145,146)*2*np.pi
    p = Pool(8)
    A = p.map(opt,g)
    p.close()
    p.join()
    ...
    np.savetxt('del_eta_time.txt',time)
    np.savetxt('del_eta_fid.txt',fid)
    figure()
    plot(g/2/np.pi,time)
    xlabel('del_eta/2/pi')
    ylabel('omega')
